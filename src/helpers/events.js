...
const dateGenerator = (startDate, startTime, endDate, endTime, days, habit) => {
  ...
  let event = {
    ...
    //possible keys for other features in calendar
    // state: null,
    // attendees: null,
    // isPrivate: false,
  ...
  let repeatDates = repeatDaysArray.map((day) => {
    let num =
      weekObject[day] === startDateNum ? 0 : weekObject[day] - startDateNum;
    return moment(startDate, "YYYY-MM-DDTHH:mm:ss").add(num, "days");
    // }
  });

  let duration = Math.round(
    moment(endDate, "YYYY-MM-DDTHH:mm:ss").diff(
  ...
  for (let i = 0; i <= duration; i++) {
    ...
    repeatDates.forEach((date) => {
      let newStartDate = moment(date, "YYYY-MM-DDTHH:mm:ss")
        .add(7 * i, "days")
        .add(startTime, "time")
        .format("YYYY-MM-DDTHH:mm:ss");
      let newEndDate = moment(date, "YYYY-MM-DDTHH:mm:ss")
        .add(7 * i, "days")
        .add(endTime, "time")
        .format("YYYY-MM-DDTHH:mm:ss");
      let updatedEvent = {
        ...event,
        unique_event_id: randomEventId(),
        start: newStartDate,
        end: newEndDate,
      };
...
const generateEvents = (habit) => {
  // Use sunday as the reference point to change the day
  ...
  return dateGenerator(
    ...
    (item) => item.start >= habit.start_date && item.end <= habit.end_date
  );
};
...
const countCompleteEvents = (habits, events) => {
  ...
  habits.forEach((habit) => {
    ...
    let filteredEventArray = events.filter(
      (event) => event.habit_id === habit.id
    ); // find all the events for the habit

    filteredEventArray.forEach((event) => {
      if (event.completed) {
        completedCount++;
      } else {
        incompletedCount++;
