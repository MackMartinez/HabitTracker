...
export default function HabitList(props) {
  let pastHabits = props.state.habits
    .filter(
      (habit) =>
        moment(props.date, "YYYY-MM-DDTHH:mm:ss").diff(
          moment(habit.end_date, "YYYY-MM-DDTHH:mm:ss"),
          "days"
        ) >= 0
    )
    .map((habit, index) => {
  ...
  let futureHabits = props.state.habits
    .filter(
      (habit) =>
        moment(props.date, "YYYY-MM-DDTHH:mm:ss").diff(
          moment(habit.end_date, "YYYY-MM-DDTHH:mm:ss"),
